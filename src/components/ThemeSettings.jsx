...
export default function ThemeSettings() {
  ...
  const {setMode, setColor, setThemeSettings, currentColor, currentMode} = useStateContext()
            ...
            {themeColors.map((item, index) => (
              <TooltipComponent key={index} content={item.name}>
                <div className='relative mt-2 cursor-pointer flex gap-5 item-center'>
                  <button type='button' className='h-10 w-10 rounded-full cursor-pointer' style={{backgroundColor: item.color}} onClick={()=>setColor(item.color)}><BsCheck className={`ml-2 text-2xl text-white ${currentColor===item.color ? 'block' : 'hidden'}`}/></button>
                </div>
              </TooltipComponent>
            ))}
          </div>
